...
# 2. Montar archivos estáticos (CSS, JS, Imágenes)
app.mount("/frontend", StaticFiles(directory="frontend"), name="frontend")

# 3. Rutas del Frontend (Navegación HTML)
@app.get("/")
def serve_root():
...
def api_consumo(
    ...
    query = """
        SELECT
            t.anio, t.bimestre, NULL AS fecha_referencia, u.alcaldia, u.colonia,
    ...
    return {
        ...
        "data": [
            {
                "anio": r[0], "bimestre": r[1], "fecha": str(r[2]) if r[2] else None,
                "alcaldia": r[3], "colonia": r[4], "indice_des": r[5],
                "consumo_total": float(r[6]) if r[6] else 0, "consumo_prom": float(r[7]) if r[7] else 0,
                "consumo_total_dom": float(r[8]) if r[8] else 0, "consumo_prom_dom": float(r[9]) if r[9] else 0,
                "consumo_total_mixto": float(r[10]) if r[10] else 0, "consumo_total_no_dom": float(r[11]) if r[11] else 0,
            }
            for r in rows
        ]
    }
...
def api_resumen():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT u.alcaldia, u.colonia, SUM(f.consumo_total) as total_agua, AVG(f.consumo_prom) as promedio
        FROM fact_consumo_agua f
        JOIN dim_ubicacion u ON f.id_ubicacion = u.id_ubicacion
    ...
    return [{"alcaldia": r[0], "colonia": r[1], "total_agua": float(r[2]) if r[2] else 0, "promedio": float(r[3]) if r[3] else 0} for r in rows]
...
def api_top_consumo(limit: int = 10):
    ...
    return [{"colonia": r[0], "alcaldia": r[1], "total_agua": float(r[2]) if r[2] else 0} for r in rows]
...
def api_indices():
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT id_indice_des, indice_des FROM dim_indice_des WHERE indice_des IS NOT NULL ORDER BY indice_des;")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [{"id_indice_des": r[0], "indice_des": r[1]} for r in rows]
...
def api_colonias(alcaldia: str = None):
    ...
    cur = conn.cursor()
    if alcaldia:
        cur.execute("SELECT DISTINCT colonia FROM dim_ubicacion WHERE alcaldia ILIKE %s AND colonia IS NOT NULL ORDER BY colonia;", (f"%{alcaldia}%",))
    else:
        cur.execute("SELECT DISTINCT colonia FROM dim_ubicacion WHERE colonia IS NOT NULL ORDER BY colonia;")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [r[0] for r in rows]